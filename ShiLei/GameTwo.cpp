...
Game::Game()
{
	...
	if (!winTexture.loadFromFile("winner.png", m_renderer))
	{
		printf("Failed to load dot texture!\n");
	}
	if (!loseTexture.loadFromFile("lose.png", m_renderer))
	{
		printf("Failed to load dot texture!\n");
	}
	if (client->run())
...
void Game::update()
{
	...
	if (client->collide && m_dot->isChaser)
	{
		client->sendMessage("Game,collide,");
		endTextue = winTexture;
		gameover = true;
	}
	else if (client->collide && !m_dot->isChaser)
	{
		client->sendMessage("Game,collide,");
		endTextue = loseTexture;
		gameover = true;
	}
...
void Game::render()
{
	...
	if (gameover == true)
	{
		endTextue.render(0, 0, m_renderer);
	}
