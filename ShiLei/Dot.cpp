...
void Dot::handleEvent(SDL_Event& e, Client & client)
{
	if (isChaser)
	{
...
void Dot::move(int SCREEN_HEIGHT, int SCREEN_WIDTH, Client & client)
{
	...
	message = "Position," + std::to_string(mPosX) + "," + std::to_string(mPosY) + ",";
...
std::string Dot::GetPosAsString()
{
	...
	return std::string(std::to_string(mPosX) + "," + std::to_string(mPosY));
}
